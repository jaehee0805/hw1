	...
	for _, test := range tests {
		func() {
			...
			if ans_sequence_exists != test.expectedSequenceExists || !reflect.DeepEqual(ans_sequence, test.expectedSequence) {
				message := fmt.Sprintf(
					"FindSequence failed on (%s, %d, %d, %d); expected: (%q, %t), got (%q, %t).",
					test.testID, test.source, test.target, test.sequenceLength,
					test.expectedSequence, test.expectedSequenceExists,
					ans_sequence, ans_sequence_exists,
				)
				t.Errorf(message)
			}
		}()
		...
		func() {
			...
			if !ans_sequence_exists || (!reflect.DeepEqual(ans_sequence, test.expectedSequence1) && !reflect.DeepEqual(ans_sequence, test.expectedSequence2)) {
				message := fmt.Sprintf(
					...
					test.testID, test.source, test.target, test.sequenceLength,
					test.expectedSequence1, test.expectedSequenceExists,
					test.expectedSequence2, test.expectedSequenceExists,
					ans_sequence, ans_sequence_exists,
				)
				t.Errorf(message)
			}
	...
	for _, test := range tests {
		func() {
			...
			if !ans_sequence_exists ||
				...
				message := fmt.Sprintf(
					"FindSequence failed on (%s, %d, %d, %d); expected any one of the following results: (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t), "+
						"got (%q, %t).",
					test.testID, test.source, test.target, test.sequenceLength,
					test.expectedSequence1, test.expectedSequenceExists,
					test.expectedSequence2, test.expectedSequenceExists,
					test.expectedSequence3, test.expectedSequenceExists,
					test.expectedSequence4, test.expectedSequenceExists,
					test.expectedSequence5, test.expectedSequenceExists,
					ans_sequence, ans_sequence_exists,
				)
				t.Errorf(message)
			}
	...
	for _, test := range tests {
		func() {
			...
			if !ans_sequence_exists ||
				...
				message := fmt.Sprintf(
					"FindSequence failed on (%s, %d, %d, %d); expected any one of the following results: (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t) or (%q, %t), "+
						"got (%q, %t).",
					test.testID, test.source, test.target, test.sequenceLength,
					test.expectedSequence1, test.expectedSequenceExists,
					test.expectedSequence2, test.expectedSequenceExists,
					...
					test.expectedSequence4, test.expectedSequenceExists,
					test.expectedSequence5, test.expectedSequenceExists,
					test.expectedSequence6, test.expectedSequenceExists,
					test.expectedSequence7, test.expectedSequenceExists,
					test.expectedSequence8, test.expectedSequenceExists,
					ans_sequence, ans_sequence_exists,
				)
				t.Errorf(message)
			}
